...
class CtaEngine(object):
    ...
    def processTickEvent(self, event):
        ...
        if tick.vtSymbol in self.tickStrategyDict:
            ...
            #ROBINLIN self.writeCtaLog(u'ctaEngine Get a TICK')
            try:
                # 添加datetime字段
                ...
                return
                #tick.datetime = datetime.now(timezone('Asia/Shanghai'))#.strftime('%Y%m%d %H:%M:%S.%f')
                #print("time error,fill in tick.datetime:" + str(tick.datetime))
    ...
    def processInstrumentEvent(self, event):
        # print "processInstrumentEvent:: GOT EVENT_INSTRUMENT"
        instrument = event.dict_['data']
        for name in self.strategyDict.keys():
            strategy = self.strategyDict[name]
            if strategy.inited:
                self.callStrategyFunc(strategy, strategy.onInstrument, instrument)
    #----------------------------------------------------------------------
    ...
    def saveOrderData(self, strategy, order):  #processOrderEvent
        ...
        # d = copy(flt)
        # #self.strategyOrderDict
        # for key in strategy.strategyOrderDict.keys():
        #      #d[key] = strategy.__getattribute__(key)
        # #
        # self.mainEngine.dbUpdate(ORDER_DB_NAME, strategy.className,d, flt, True)
        # # content = '策略%s同步数据保存成功，当前持仓%s' %(strategy.name, strategy.pos)
        # # self.writeCtaLog(content)
    ...
    def cancelAll(self, name):
        ...
        for orderID in list(s):
            self.writeCtaLog("cancelAll::cancel{}".format(orderID))
